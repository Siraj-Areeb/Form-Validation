function validateForm() {
  let formElement = document.querySelector(".registration_form");
  ...
  let validationCheckPoints = [
    {
      attribute: "match",
      isValid: (input) => {
        let matchSelector = input.getAttribute("match");
        let matchElement = document.querySelector(`#${matchSelector}`);
        return matchElement && matchElement.value.trim() == input.value.trim();
      },
      errorMessage: (input, label) =>
        `${label.textContent} should match Password.`,
    },
